...
import { Check, Search, X, SearchX } from 'lucide-react';
...
// iconNames are kebab-case; derive a display label and copy value from each
const ALL_ICONS = [...iconNames].sort();

function toPascalCase(str) {
  return str.replace(/(^|-)([a-z])/g, (_, __, c) => c.toUpperCase());
}

function IconCard({ name, onCopy, copied }) {
...
function IconsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return q ? ALL_ICONS.filter((name) => name.includes(q) || toPascalCase(name).toLowerCase().includes(q)) : ALL_ICONS;
  }, [query]);
    ...
    <div className="bg-background min-h-screen p-8 space-y-8">
      ...
      {filtered.length > 0 ? (
        <div className="grid grid-cols-[repeat(auto-fill,minmax(80px,1fr))] gap-2">
          {filtered.map((name) => (
            <IconCard key={name} name={name} onCopy={handleCopy} copied={copied} />
          ))}
        </div>
      ) : (
