...
// --sd switches between black (light) and white (dark) so shadows are always visible.
// Injected via <style> because .dark is on <html>, not inside the story wrapper.
const SHADOW_COLOR_STYLE = `
  :root { --sd: 0 0 0; }
  .dark  { --sd: 255 255 255; }
`;

const SHADOWS = [
  { label: 'shadow-none',  shadow: 'none' },
  { label: 'shadow-sm',    shadow: '0 1px 2px 0 rgb(var(--sd) / 0.08)' },
  { label: 'shadow',       shadow: '0 1px 3px 0 rgb(var(--sd) / 0.12), 0 1px 2px -1px rgb(var(--sd) / 0.12)' },
  { label: 'shadow-md',    shadow: '0 4px 6px -1px rgb(var(--sd) / 0.12), 0 2px 4px -2px rgb(var(--sd) / 0.12)' },
  { label: 'shadow-lg',    shadow: '0 10px 15px -3px rgb(var(--sd) / 0.12), 0 4px 6px -4px rgb(var(--sd) / 0.12)' },
  { label: 'shadow-xl',    shadow: '0 20px 25px -5px rgb(var(--sd) / 0.12), 0 8px 10px -6px rgb(var(--sd) / 0.12)' },
  { label: 'shadow-2xl',   shadow: '0 25px 50px -12px rgb(var(--sd) / 0.28)' },
  { label: 'shadow-inner', shadow: 'inset 0 2px 4px 0 rgb(var(--sd) / 0.08)' },
];
...
function ShadowCard({ label, shadow }) {
  return (
    <div className="space-y-4 flex flex-col items-center">
      <div
        className="w-full h-24 rounded-xl bg-card border border-border"
        style={{ boxShadow: shadow }}
      />
      <div className="w-full space-y-0.5">
        <p className="text-xs font-mono font-semibold text-foreground">{label}</p>
        <p className="text-[10px] font-mono text-muted-foreground/60 leading-relaxed break-all">
          {DESC[label]}
        </p>
      </div>
    </div>
...
export const Overview = {
  ...
  render: () => (
    <>
      <style>{SHADOW_COLOR_STYLE}</style>
      <div className="bg-background min-h-screen p-8 space-y-16">
        <div className="space-y-2 border-b border-border pb-8">
          ...
          <p className="text-muted-foreground max-w-xl mt-2">
            Tailwind's built-in shadow, blur and backdrop-blur scales. Shadow colour automatically
            switches to white in dark mode via a CSS variable.
          </p>
        </div>
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-8">
            {SHADOWS.map((s) => <ShadowCard key={s.label} {...s} />)}
          </div>
        </section>
